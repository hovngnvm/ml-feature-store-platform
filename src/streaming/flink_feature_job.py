...
logger = logging.getLogger("flink_feature_job")
...
KAFKA_DLQ_TOPIC = os.getenv("KAFKA_DLQ_TOPIC", "raw_transactions_dlq")

REDIS_HOST = os.getenv("REDIS_HOST", "localhost")
REDIS_PORT = int(os.getenv("REDIS_PORT", 6379))

# Cold Path Storage Paths (MinIO S3 / Local Parquet Data Lake)
RAW_EVENTS_PARQUET_PATH = os.path.join(PROJECT_DIR, "data", "raw_events_stream.parquet")
STREAM_DLQ_PARQUET_PATH = os.path.join(PROJECT_DIR, "data", "lakehouse", "dlq", "stream_errors.parquet")
...
def register_kafka_source(t_env: StreamTableEnvironment):
...
def upload_to_minio(file_path: str, object_name: str):
    ...
    try:
        ...
        logger.info(f"[MinIO Archival] Uploaded '{file_path}' -> MinIO S3 's3://{MINIO_BUCKET}/{object_name}'")
...
class DualPathRedisFeatureSink:
    ...
    def process_event(self, event: dict) -> dict:
        ...
        # ---------------------------------------------------------------------
        # Cold Path Archival Accumulation (Raw Event Store)
        # ---------------------------------------------------------------------
        ...
        # Compute sliding windows (1m, 5m, 1h, 24h)
    ...
    def flush_cold_path_archive(self):
        """Flushes buffered raw events to Parquet Data Lake & MinIO S3 Storage."""
        if self.raw_events_buffer:
            os.makedirs(os.path.dirname(RAW_EVENTS_PARQUET_PATH), exist_ok=True)
            df_new = pd.DataFrame(self.raw_events_buffer)
            if os.path.exists(RAW_EVENTS_PARQUET_PATH):
                try:
                    df_existing = pd.read_parquet(RAW_EVENTS_PARQUET_PATH)
            ...
            df_combined.to_parquet(RAW_EVENTS_PARQUET_PATH, index=False)
            logger.info(f"[Cold Path Archival] Saved {len(df_combined):,} raw events to '{RAW_EVENTS_PARQUET_PATH}'")
            upload_to_minio(RAW_EVENTS_PARQUET_PATH, "raw_events/stream_events.parquet")

        if self.dlq_events_buffer:
            os.makedirs(os.path.dirname(STREAM_DLQ_PARQUET_PATH), exist_ok=True)
            df_dlq = pd.DataFrame(self.dlq_events_buffer)
            df_dlq.to_parquet(STREAM_DLQ_PARQUET_PATH, index=False)
            now_dt = datetime.now(timezone.utc)
            minio_dlq_key = f"dlq/year={now_dt.year}/month={now_dt.month:02d}/stream_errors.parquet"
            logger.info(f"[DLQ Archival] Saved {len(df_dlq)} quarantined events to '{STREAM_DLQ_PARQUET_PATH}'")
            upload_to_minio(STREAM_DLQ_PARQUET_PATH, minio_dlq_key)
...
def main():
    ...
    transformed_table = transform_stream(t_env)
    ...
    logger.info(f"Starting Flink Stream Processor with DLQ & Dual-Path Archival (Broker: {KAFKA_BROKER}, Main: {KAFKA_TOPIC}, DLQ: {KAFKA_DLQ_TOPIC})...")
    try:
        ...
        logger.info(f"Kafka Broker offline ({e}). Executing Dual-Path & DLQ Simulation...")
        sink = DualPathRedisFeatureSink(dlq_producer=dlq_producer)
        sample_events = [
            {"transaction_id": 1001, "card_id": "11556", "amount": 150.0, "timestamp": "2026-07-28T12:00:00Z"},
            {"transaction_id": 1002, "card_id": "11556", "amount": -999.0, "timestamp": "2026-07-28T12:02:00Z"},  # Corrupt
            {"transaction_id": 1003, "card_id": "unknown_card", "amount": 250.0, "timestamp": "2026-07-28T12:03:00Z"} # Corrupt
        ]
